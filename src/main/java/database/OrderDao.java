...
public class OrderDao extends AbsDao<Order>{
    ...
    public int insert(Order order) {
        ...
        try {
            ...
            String sql = "INSERT INTO orders (customer_id,to_name, total, time_order,numberPhone, delivery_fee, expected_delivery_time, from_address, delivery_address, note, payment_method_id, status_id, voucher_id, hash_code) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?,?,?)";
            ...
            pst.setString(14, order.getHash());
    ...
    public int update(Order order) {
        ...
        try {
            Connection con = JDBCUtil.getConnection();
            String sql = "UPDATE orders SET customer_id=?, to_name=?, total=?, time_order=?, numberPhone=?, delivery_fee=?, expected_delivery_time=?, from_address=?, delivery_address=?, note=?, payment_method_id=?, status_id=?, voucher_id=?,hash_code=? WHERE id=?";
            ...
            pst.setString(14, order.getHash());
            pst.setInt(15, order.getId()); // Đặt tham số ID của Order cần cập nhật
    ...
    public ArrayList<Order> selectByCustomerId(int customerId) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                order.setOrderItems(orderItemDao.selectByOrderId(order));
                order.setBeforeData(order.toString());
                order.setHash(rs.getString("hash_code"));
                orders.add(order);
            }
            JDBCUtil.closeConnection(con);
        }catch (Exception e) {
