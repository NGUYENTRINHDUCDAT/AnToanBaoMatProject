...
//        });
    ...
    public KeyPairModel selectKeyPairActiveByCustomerId(int customerId){
        KeyPairModel keyPairModel = null;
        try {
            Connection con = JDBCUtil.getConnection();

            String sql = "Select * from key_pairs where customer_id = ? and status = 'active' order by id desc ";
            PreparedStatement pst = con.prepareStatement(sql);
            pst.setInt(1, customerId);
            ResultSet rs = pst.executeQuery();
            while (rs.next()) {
                keyPairModel = new KeyPairModel();
                keyPairModel.setId(rs.getInt("id"));
                keyPairModel.setCustomerId(rs.getInt("customer_id"));
                keyPairModel.setPublicKey(rs.getString("public_key"));
                keyPairModel.setStatus(rs.getString("status"));
    ...
    public static void main(String[] args) {
        KeyPairModelDao keyPairModelDao = new KeyPairModelDao();
        System.out.println(keyPairModelDao.selectKeyPairActiveByCustomerId(23).toString());
    }
}
