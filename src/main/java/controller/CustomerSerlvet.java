...
public class CustomerSerlvet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        } else if(action.equals("confirmSignature")){
            try {
                confirmSignature(req, resp);
            } catch (Exception e) {
                HttpSession session = req.getSession();
                String url = "";
                session.setAttribute("error_signature", "Chữ ký không hợp lệ");
                url = "/xacThucChuKyDienTuDatHang.jsp";
                String link = req.getScheme() + "://" + req.getServerName() + ":" + req.getServerPort()
                        + req.getContextPath();
                resp.sendRedirect(link + "/customer" +url);
            }
        } else if(action.equals("editOrder")){
            editOrder(req, resp);
        } else if(action.equals("removeOrderItem")){
            removeOrderItem(req, resp);
        } else if(action.equals("updateQuantityOnOrder")){
            updateQuantityOnOrder(req,resp);
        } else if(action.equals("confirmEditOrder")){
            try {
                confirmEditOrder(req, resp);
            } catch (Exception e) {
                throw new RuntimeException(e);
            }
        } else if(action.equals("confirmSignatureEditOrder")){
            try {
                confirmSignatureEditOrder(req, resp);
            } catch (Exception e) {
                HttpSession session = req.getSession();
                String url = "";
                session.setAttribute("error_signature", "Chữ ký không hợp lệ");
                url = "/xacThucChuKyDienTuChinhSuaDonHang.jsp";
                String link = req.getScheme() + "://" + req.getServerName() + ":" + req.getServerPort()
                        + req.getContextPath();
                resp.sendRedirect(link + "/customer" +url);
            }
        } else if(action.equals("historyOrder")){
            try {
                historyOrder(req,resp);
            } catch (Exception e) {
                throw new RuntimeException(e);
            }
        } else if(action.equals("confirmSignatureImportNewKey")){
            try {
                confirmSignatureImportNewKey(req,resp);
            } catch (Exception e) {
                HttpSession session = req.getSession();
                String url = "";
                session.setAttribute("error_signature", "Chữ ký không hợp lệ");
        ...
        } else if(action.equals("confirmSignatureCreateNewKey")){
            try {
                confirmSignatureCreateNewKey(req,resp);
            } catch (Exception e) {
                HttpSession session = req.getSession();
                String url = "";
                session.setAttribute("error_signature", "Chữ ký không hợp lệ");
                url = "/xacThucChuKyDienTuTaoKhoaMoi.jsp";
                String link = req.getScheme() + "://" + req.getServerName() + ":" + req.getServerPort()
                        + req.getContextPath();
                resp.sendRedirect(link + url);
            }
    ...
    private void register(HttpServletRequest req, HttpServletResponse resp) throws Exception {
        ...
        if (!error) {
            ...
            try {
                ...
                String encryptedFilePath = getServletContext().getRealPath("/") + "encrypted_private_key_" + customer.getUsername() + ".txt";
                try (PrintWriter writer = new PrintWriter(filePath)) {
                    writer.write(ds.exportPrivateKey());
                }
                File privateKeyFile = new File(filePath);
                File encryptedFile = new File(encryptedFilePath);
                AES.encryptFile(privateKeyFile, encryptedFile);
                Email.sendEmailPrivateKey(customer, encryptedFile);
                req.setAttribute("privateKeyFilePath", "encrypted_private_key_" + username + ".txt");
...
////            }
            ...
            String filePath = getServletContext().getRealPath("/") + "hash_code" + "_order_" + order.getCustomer().getFullName() + ".txt";
            try (PrintWriter writer = new PrintWriter(filePath)) {
                writer.write(hash);
            }
            session.setAttribute("hashCodeFile", "hash_code" + "_order_" + order.getCustomer().getFullName() + ".txt");
            String link = req.getScheme() + "://" + req.getServerName() + ":" + req.getServerPort()
                    + req.getContextPath();
            resp.sendRedirect(link + "/customer/xacThucChuKyDienTuDatHang.jsp");
        }
    }
    ...
    private void forgetPassword(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if(!cusDao.checkUsername(username)){
            ...
            Email.sendEmaiQuenMatKhau("Quen Mat Khau",customer, resetCode);
    ...
    private void confirmKeyPairs(HttpServletRequest req, HttpServletResponse resp) throws Exception{
        ...
        HttpSession session = req.getSession();
        Customer customer = (Customer) session.getAttribute("customer_login");
        String publicKey = req.getParameter("publicKey").trim();
    ...
    private void confirmSignature(HttpServletRequest req, HttpServletResponse resp) throws Exception {
        req.setCharacterEncoding("UTF-8");
        resp.setCharacterEncoding("UTF-8");
        resp.setContentType("text/html; charset=UTF-8");
        System.out.println("Da Vao Day");
        String hash = req.getParameter("hash_code").trim();
        String signature = req.getParameter("digital_signature").trim();
        HttpSession session = req.getSession();
        Customer customer = (Customer) session.getAttribute("customer_login");
        ...
        if (keyPairModel != null){
            ...
            if(ds.verifySignature(hash, signature)){
                Order order = (Order) session.getAttribute("order");
                List<CartItem> cartItems = (List<CartItem>) session.getAttribute("selectedCartItems");
                for (CartItem cartItem : cartItems) {
                    cartItemDao.delete(cartItem);
                ...
                session.setAttribute("error_signature", "Chữ kí không hợp lệ");
            }
        }else{
            session.setAttribute("error_signature", "Cặp khóa của bạn đã bị thu hồi, vui lòng tạo cặp khóa mới để có thể đặt hàng");
    ...
    private void editOrder(HttpServletRequest req, HttpServletResponse resp) throws IOException, ServletException {
        req.setCharacterEncoding("UTF-8");
        resp.setCharacterEncoding("UTF-8");
        resp.setContentType("text/html; charset=UTF-8");
        HttpSession session = req.getSession();
        ...
        int orderId = Integer.parseInt(orderIdParam);
        Order order = orderDao.selectById(orderId);
        session.setAttribute("order_edit", order);
        req.setAttribute("orderDetail", order);
        req.getRequestDispatcher("/customer/chinhSuaDonHang.jsp").forward(req, resp);
    }
    private void removeOrderItem(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        req.setCharacterEncoding("UTF-8");
        resp.setCharacterEncoding("UTF-8");
        resp.setContentType("application/json; charset=UTF-8");
        HttpSession session = req.getSession();
        Customer customer = (Customer) req.getSession().getAttribute("customer_login");
        String orderItemId = req.getParameter("orderItem");
        Order order = (Order) session.getAttribute("order_edit");
        for (OrderItem orderItem : order.getOrderItems()){
            if (orderItem.getId() == Integer.parseInt(orderItemId)){
        ...
        Gson gson = new Gson();
        jsonResponse.addProperty("success", true);
        jsonResponse.addProperty("priceProduct", order.priceProduct());
        jsonResponse.addProperty("totalPrice", order.priceProduct() + order.getDeliveryFee());
        resp.getWriter().write(gson.toJson(jsonResponse));
    }
    private void updateQuantityOnOrder(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        req.setCharacterEncoding("UTF-8");
        resp.setCharacterEncoding("UTF-8");
        resp.setContentType("application/json; charset=UTF-8");
        ...
        Order order = (Order) session.getAttribute("order_edit");
        if (order == null) {
            resp.sendError(HttpServletResponse.SC_BAD_REQUEST, "Không tìm thấy đơn hàng cần chỉnh sửa.");
            return;
        }

        String orderItemId = req.getParameter("orderItem");
        String updateAction = req.getParameter("update");
        if (orderItemId == null || updateAction == null) {
        ...
        while (iterator.hasNext()) {
            ...
            if (orderItem.getId()==(Integer.parseInt(orderItemId))) {
                itemFound = true;
                if ("plus".equalsIgnoreCase(updateAction)) {
                    orderItem.setQuantity(orderItem.getQuantity() + 1);
                    quantity = orderItem.getQuantity();
                } else if ("minus".equalsIgnoreCase(updateAction)) {
                    orderItem.setQuantity(orderItem.getQuantity() - 1);
                    quantity = orderItem.getQuantity();
        ...
        if (!itemFound) {
            resp.sendError(HttpServletResponse.SC_BAD_REQUEST, "Không tìm thấy sản phẩm trong đơn hàng.");
            return;
        }
        ...
        double priceProduct = order.priceProduct();
        double totalPrice = priceProduct + order.getDeliveryFee();
        ...
        jsonResponse.addProperty("priceProduct", priceProduct);
        jsonResponse.addProperty("totalPrice", totalPrice);

        resp.getWriter().write(gson.toJson(jsonResponse));
    }

    private void confirmEditOrder(HttpServletRequest req, HttpServletResponse resp) throws Exception {
        req.setCharacterEncoding("UTF-8");
        resp.setCharacterEncoding("UTF-8");
        resp.setContentType("application/json; charset=UTF-8");
        HttpSession session = req.getSession();
        ...
        order.setTotal(order.priceProduct() + order.getDeliveryFee());
        StringBuilder sb = new StringBuilder();
        List<OrderItem> orderItems = new ArrayList<>(order.getOrderItems());
        orderItems.sort(new Comparator<OrderItem>() {
            @Override
        ...
        String link = req.getScheme() + "://" + req.getServerName() + ":" + req.getServerPort()
                + req.getContextPath();
        resp.sendRedirect(link + "/customer/xacThucChuKyDienTuChinhSuaDonHang.jsp");

    }
    private void confirmSignatureEditOrder(HttpServletRequest req, HttpServletResponse resp) throws Exception {
        req.setCharacterEncoding("UTF-8");
        resp.setCharacterEncoding("UTF-8");
        resp.setContentType("text/html; charset=UTF-8");
        System.out.println("Da Vao Day");
        ...
        KeyPairModel keyPairModel = keyPairModelDao.selectKeyPairActiveByCustomerId(customer.getId());
        String url = "/xacThucChuKyDienTuChinhSuaDonHang.jsp";
        System.out.println("Hash: " + hash);
        System.out.println("Signature: "+ signature);
        System.out.println(keyPairModel.getPublicKey());
        if (keyPairModel != null){
            ...
            if(ds.verifySignature(hash, signature)){
                ...
                order.setHash(hash);
                order.setTotal(order.priceProduct() + order.getDeliveryFee());
                Order orderOld = orderDao.selectById(order.getId());
                for (OrderItem orderItem: orderOld.getOrderItems()){
                ...
                orderDao.update(order);
                url = "/chinhSuaDonHangThanhCong.jsp";
            }else {
                session.setAttribute("error_signature", "Chữ kí không hợp lệ");
            }
        }else{
    ...
    private void historyOrder(HttpServletRequest req, HttpServletResponse resp) throws Exception {
        req.setCharacterEncoding("UTF-8");
        resp.setCharacterEncoding("UTF-8");
        resp.setContentType("text/html; charset=UTF-8");
        System.out.println("Da Vao Day");
        String url = "/lichSuMuaHang.jsp";
        HttpSession session = req.getSession();
        Customer customer = (Customer) session.getAttribute("customer_login");

        List<Order> orders = orderDao.selectByCustomerId(customer.getId());
        ...
        for (Order order: orders){
           if(order.getStatus().getId() < 5){
               ...
               System.out.println("message order:" + message);
               if(!hash.equals(order.getHash())){

                   order.setStatus(orderStatusDao.selectById(6));
                   orderDao.update(order);
                   Email.sendEmailCanhBaoDonHang(order);
               }
           }
        }
        orders = orderDao.selectByCustomerId(customer.getId());
        session.setAttribute("history_orders", orders);
    ...
    private void confirmSignatureImportNewKey(HttpServletRequest req, HttpServletResponse resp) throws Exception {
        req.setCharacterEncoding("UTF-8");
        resp.setCharacterEncoding("UTF-8");
        resp.setContentType("text/html; charset=UTF-8");
        System.out.println("Da Vao Day");
        ...
        KeyPairModel keyPairModel = keyPairModelDao.selectKeyPairActiveByCustomerId(customer.getId());
        String url = "/xacThucChuKyDienTuNhapKhoaMoi.jsp";
        System.out.println("Hash: " + hash);
        System.out.println("Signature: "+ signature);
        System.out.println(keyPairModel.getPublicKey());
        if (keyPairModel != null){
    ...
    private void confirmSignatureCreateNewKey(HttpServletRequest req, HttpServletResponse resp) throws Exception {
        req.setCharacterEncoding("UTF-8");
        resp.setCharacterEncoding("UTF-8");
        resp.setContentType("text/html; charset=UTF-8");
        System.out.println("Da Vao Day");
        ...
        KeyPairModel keyPairModel = keyPairModelDao.selectKeyPairActiveByCustomerId(customer.getId());
        String url = "/xacThucChuKyDienTuTaoKhoaMoi.jsp";
        System.out.println("Hash: " + hash);
        System.out.println("Signature: "+ signature);
        System.out.println(keyPairModel.getPublicKey());
        if (keyPairModel != null){
            ...
            if(ds.verifySignature(hash, signature)){
                ...
                session.setAttribute("publicKey", ds.exportPublicKey());
                ...
                AES.encryptFile(privateKeyFile, encryptedFile);
                Email.sendEmailPrivateKey(customer, encryptedFile);
                session.setAttribute("privateKeyFilePath", "encrypted_private_key_" + customer.getUsername() + ".txt");
