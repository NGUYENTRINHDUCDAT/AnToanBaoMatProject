...
<head>
    ...
    <style type="text/css">
        ...
        table th, table td {
            border: 2px solid #cccccc; /* Đường viền đậm */
            padding: 8px;
        }

        table tr:nth-child(even) {
            background-color: #e0e0e0; /* Màu nền xen kẽ */
        }
...
<div class="container">
    ...
    <ul class="nav nav-pills mb-3" id="pills-tab" role="tablist">
        ...
        <li class="nav-item" role="presentation">
            <a class="a" href="../customer?action=getOrderStatus&status=6"><button class="nav-link" id="edited-orders" data-bs-toggle="pill" type="button" role="tab" aria-controls="cancelled-orders" aria-selected="false"> Đã bị chỉnh sửa </button></a>
        </li>
        <li class="nav-item" role="presentation">
            <a class="a" href="../customer?action=getOrderStatus&status=7"><button class="nav-link" id="refund-orders" data-bs-toggle="pill" type="button" role="tab" aria-controls="cancelled-orders" aria-selected="false"> Y/c hoàn tiền </button></a>
        </li>
    </ul>
    <h4 class="text">
        <span> ${emtyOrder} </span>
            ...
            <div class="card-body">
                ...
                <div class="table-responsive">
                    ...
                    <table class="styled-table">
                        ...
                        <c:forEach items="${history_orders}" var="p">
                            <tr>
                                ...
                                <td class="text-center align-middle"> <a class="a" href="../customer?action=detailOrder&orderId=${p.id}"><button class="btn btn-success"> Chi tiết </button></a>
                                    <c:if test="${p.status.id < 3|| (p.status.id ==6 && p.paymentMethod.id ==1)}">
                                    <a class="a" href="../customer?action=updateOrder&status=5&orderId=${p.id}"><button class="btn btn-success"> Hủy </button></a>
                                </c:if>
                                    <c:if test="${p.status.id == 6 && p.paymentMethod.id ==2}">
                                        <a class="a" href="../customer?action=updateOrder&status=7&orderId=${p.id}"><button class="btn btn-success"> Y/c Hoàn tiền </button></a>
                                    </c:if>
                                    <c:if test="${p.status.id < 2}">
                                        <a class="a" href="../customer?action=editOrder&orderId=${p.id}"><button class="btn btn-success"> Chỉnh Sửa </button></a>
                                    </c:if>
